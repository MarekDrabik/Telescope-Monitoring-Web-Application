...
import { creation, showSide, showSelector } from "../../animations/animations";
...
export class DygraphsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  previousTelemetryCache: TelemetryCachePointWithDate[] = [];
  ...
  private _registerCallbacks() {
    this.dygraph.updateOptions({
      drawCallback: () => {
        ...
        if (this.dygraph.isZoomed()) {
          ...
          if (this.dygraph.isZoomed("x")) {
            ...
            if (x0 > xAxisExtremes[0]) {
              ...
              //if specifically this side is unzoomed
            ...
            //graph is unzoomed horizontally and zoomed vertically
            this.showLeftSide = "hide";
            this.showRightSide = "hide";
          }
          ...
          if (this.dygraph.isZoomed("y")) {
            ...
            this.zoomedVertically = true;
            this.showVerticalSides = "show";
          } else {
            //graph is unzoomed vertically and zoomed horizontally
            this.zoomedVertically = false;
            this.showVerticalSides = "hide";
          }
        } else {
          //graph totally unzoomed. this 'else' is here only to fix bug where horizontal zoom
        ...
        this.fixLegendPositionService.modifyLegendPositionAccordingly(
          properPosition
        );
      },
      zoomCallback: (x0, x1, yRange) => {
        ...
        if (this.dygraph.isZoomed() && this.isFreshZoomIn) {
          ...
          this.showInfoTimeout = setTimeout(
            () => (this.showInfo = false),
            this.SHOW_INFO_DURATION
          );
        }

        if (!this.dygraph.isZoomed()) {
          //on total unzoom:
          if (this.zoomTracker.unzoomedFromDblclick) {
            //full unzoom is intentional, nothing to tinker,
            //just reset the property
            this.zoomTracker.unzoomedFromDblclick = false;
          } else if (this.zoomTracker.lastYRange !== null) {
            //total unzoom was triggered by horizontal rangeselector,
            // but there was a vertical zoom before that should be kept, so restore it:
            this.dygraph.updateOptions({
              valueRange: this.zoomTracker.lastYRange,
            });
  ...
  private _subscribeToVerticalInputSubject() {
    //called once on init
    this.verticalInputSubject = new Subject<any>();

    this._subs.push(
      this.verticalInputSubject
  ...
  private _updateSelectors(what: Array<string>) {
    // 'value' / 'step'
    ...
    [step, roundingCoeficient] = this.utils.calculateStepAndPrecision(
      currentRange,
      this.ZOOM_RATE
    );
    //patch value is here for the case where user zooms in with mouse drag
    let newYAxisRange = [
      +yAxisRange[0].toFixed(roundingCoeficient),
      +yAxisRange[1].toFixed(roundingCoeficient),
    ];
    if (what.includes("value")) {
      this.lowerBound = newYAxisRange[0];
      this.upperBound = newYAxisRange[1];
    }
    if (what.includes("step")) {
  ...
  onUnzoomFromVerticalRefreshButton() {
    ...
    this.showVerticalSides = "hide";
    if (!this.dygraph.isZoomed("x")) {
      //its a total unzoom, so update necessary variables
      this.showInfo = false; //unload immediatly (useful if unzoom happened while info was still displayed)
  ...
  private _subscribeToParentUpdates() {
    const _sub = this.updateDygraphSubject.subscribe((options) => {
      let opt: DygraphOptions = {};
      if (options.withCheck) {
        //check to be implemnted if needed
      ...
      if (this.dygraph.isZoomed("x") && opt.hasOwnProperty("file")) {
        if (!this._zoomShouldMove(opt["file"], this.previousTelemetryCache)) {
      ...
      //empty dataset
      if (
        opt.hasOwnProperty("file") &&
        Array.isArray(opt["file"]) &&
        opt["file"].length === 0
      ) {
        opt["file"] = [];
      }
      if (opt.hasOwnProperty("file")) {
        this.previousTelemetryCache = opt["file"].slice();
      }
  ...
  private _calculateNewZoomWindowPosition(
    cache: TelemetryCachePointWithDate[]
  ) {
    if (cache.length < 2) {
      return null;
    } //not enough data
    let lastTwoTimestamps = cache
      .slice(cache.length - 2)
      .map((datum) => datum[0].getTime());
    let deltaTimeOfMostRecentUpdate = Math.abs(
      lastTwoTimestamps[0] - lastTwoTimestamps[1]
    );
    return this.dygraph
      .xAxisRange()
      .map((extreme) => extreme + deltaTimeOfMostRecentUpdate);
  }

  private _zoomShouldMove(newCache, previousTelemetryCache) {
    //fixing bug where horizontal zoom of graph moved too fast on updates
    let l1 = previousTelemetryCache.length;
    let l2 = newCache.length;
    if (l1 === 0 || l1 === 1 || l2 === 0 || l2 === 1) {
      return true;
    }
    // when earliest telemetry values differ it means it was updatedByTelemetryUpdate
    let updatedByTelemetryUpdate =
      previousTelemetryCache[l1 - 1][0] !== newCache[l2 - 1][0];
    return !updatedByTelemetryUpdate;
  }
