...
export class ClockService {
  ...
  constructor() {
    ...
    this.clock = new Observable<number>(observer => {
      ...
      function setUpdateInterval () {
        ...
        periodicUpdate = setInterval(() => {
          if (observer.closed) {clearInterval(periodicUpdate)}
          observer.next(Date.now())

          dateNowString = Date.now().toString()
          dateNowMilliseconds = +dateNowString.slice(dateNowString.length - 3)
          if (dateNowMilliseconds < 0 || dateNowMilliseconds > 1000) { console.error ('milliseconds invalid')}
          if (dateNowMilliseconds > 20) { //!== 0 would be ideal, but there is always some delay caused by processing,
    ...
    this.milliClock = new Observable<number>(observer => {
      let dateNow: number; let tenth: number;
      function reset() { //implemented to hopefully save performance to call 
