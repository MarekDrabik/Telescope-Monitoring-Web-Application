...
export class AppletLayoutTrackerDirective implements OnDestroy {
  ...
  constructor(
    ...
    this.mouseDownUnlisten = this.ren2.listen(appletRef.nativeElement, 'mousedown', () => {
      let mouseUpUnlisten = this.ren2.listen(document, 'mouseup', () => {
        ...
        if (this._appletPositionHasChanged(appletPosition)) {
          this.layoutService.storeAppletPosition(this.appletId, appletPosition)
        }
        //applet's size:
  ...
  private _appletPositionHasChanged(actualPosition) {
    let storedPosition = this.layoutService.getAppletDetailsProperty(this.appletId, 'position')
    if (actualPosition.x === storedPosition.x && actualPosition.y === storedPosition.y) {
      return false;
    }
    return true;
  }
  private _appletSizeHasChanged(actualSize) {
    let storedSize = this.layoutService.getAppletDetailsProperty(this.appletId, 'size')
    if (storedSize.width === actualSize.width && storedSize.height === actualSize.height) {
      return false;
    }
    return true;
  }
