...
import { WebsocketMessageUnwrappingService } from "./websocket-message-unwrapping.service";
...
export class WebsocketSubscriptionService {
  ...
  ws = webSocket({
    ...
    deserializer: (message) => {
      ...
      return this.websocketMessageUnwrappingService.unwrapMessageBasedOnType(
        message.data
      );
    },
    openObserver: {
    ...
    closeObserver: {
      next: (close) => {
        if (close.code === 4001) {
          this.connectionBrokenService.onExpiredSession();
        }
      },
    },
  });
  ...
  constructor(
    private websocketMessageUnwrappingService: WebsocketMessageUnwrappingService,
    private telemetrySourcesService: TelemetrySourcesService,
    private connectionBrokenService: ConnectionBrokenService
  ) {
    for (let sourceName of this.telemetrySourcesService.getAllRequestableSources()) {
      ...
      /*(allPoints/allPositions = all sources 'telemetrySource' are always requested for table/graph/map applet for simplicity,
      single source functionality is available but not needed because the size of data is very small to be
      bothered requesting them separately)*/

      WebsocketSubscriptionService.subscriptors[sourceName] = this.ws
        .multiplex(
  ...
  subscribe(sourceName: TelemetrySource | TelemetrySourceGroup, callback) {
    ...
    let sub = WebsocketSubscriptionService.subscriptors[sourceName].subscribe(
      ...
      (error) => {
        ...
        this.connectionBrokenService.onConnectionBroken();
      }
    );
    sub.add(() => {
      // rxjs function adds teardown function that will be called on unsubscription
  ...
  private _socketSubscriptionManager(action: "add" | "remove") {
    ...
    if (action === "add") {
      ...
      if (!this.captainSubscriber) {
        //means there is no connection, so open it by subscribing the captain:
    ...
    if (action === "remove") {
      if (--this.subscribers < 0) {
        console.error(
          "No subscriber to pop from list. There was suppose to be at least one!"
        );
      }
      if (this.subscribers === 0) {
        //if this was the last subscriber, then we will close the socket in few seconds
