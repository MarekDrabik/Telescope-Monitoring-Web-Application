...
export class MoveResizeDirective implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.submenuComponent.moveResizeTrigger.subscribe(optionChecked => {
      if (!this.alreadyListening && optionChecked === true) {
  ...
  private _listenForMoveGrab() {
    this.unlistenGrab = this.renderer2.listen(this.moveHandle, 'mousedown', (event) => {
      if (!this.submenuComponent.isFullscreen) {
        ...
        var unlistenMove = this.renderer2.listen(document, 'mousemove', (event) => {
          ...
          // this.options = { ...this.options, didUserResize: true}
