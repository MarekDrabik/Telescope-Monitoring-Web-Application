...
export class ThumbnailComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this._renderImage();
    this.focuseImageSubscription = this.focusedImageService.focusedImageSubject.subscribe(
      (data) => {
        if (data.point) {
          //null/undefined on init and when no image available
  ...
  onClick(event: Event) {
    ...
    //focus this thumbnail on user click
    this.focusedImageService.focusedImageSubject.next({
      point: this.telemetry,
      userSelected: true,
    });
  }
  ...
  private _renderImage() {
    this.imageUrl = this.sanitizer.bypassSecurityTrustUrl(
      URL.createObjectURL(this.telemetry[1])
    ) as string;
    let timestamp = this.utilitiesService.timestampEpochnumberToDateTime(
      +this.telemetry[0],
      "24"
    );
