...
export class FixLegendPositionService {
  ...
  initiate(appDygraphElement, render) {
    this.dygraphLegendElement = this._getDygraphLegendElement(appDygraphElement);
  ...
  private _fixLegendPositionContinuously(appDygraphElement) {
    this.dygraphsRenderer.listen(appDygraphElement.nativeElement, 'mousemove', (mouseEvent) => {
      let chartHeight = appDygraphElement.nativeElement.offsetHeight;
      let mouseY = mouseEvent.offsetY
      if (mouseY > chartHeight / 2) {
        if (this.dygraphLegendElement.style['top'] === '0px' || this.dygraphLegendElement.style['top'] === '') {
          this.legendPositionSubject.next('top')
        }
      } else {
        if (this.dygraphLegendElement.style['bottom'] === '0px' || this.dygraphLegendElement.style['bottom'] === '') {
  ...
  modifyLegendPositionAccordingly = (position) => {
    if (position === 'top') {
      ...
      this.dygraphsRenderer.setStyle(this.dygraphLegendElement, 'bottom', '50px')
    }
  }
  ...
  private _getDygraphLegendElement(appDygraphElement) {
    let dygraphLegendElement = Array.from(
      Array.from(
        Array.from(
          (Array.from(appDygraphElement.nativeElement.children)[0] as HTMLElement).children
        )[0].children
      )[0].children
    ).find((value) => {
