...
export class FadeVerticalSelectorsDirective implements OnInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      changes.chartIsZoomedVertically &&
      !changes.chartIsZoomedVertically.firstChange
    ) {
  ...
  private _observeAndAct() {
    ...
    this.coreSubscription = this.coreObservable.subscribe((event) => {
      if (event === "mouseStartedMoving") {
        state.mouseMoving = true;
      }
      if (event === "mouseStoppedMoving") {
        state.mouseMoving = false;
      }
      if (event === "mouseOverChart") {
        state.mouseOverChart = true;
      }
      if (event === "mouseOutChart") {
        state.mouseOverChart = false;
      }
      if (event === "mouseOverTop") {
        state.mouseOverTop = true;
      }
      if (event === "mouseOutTop") {
        state.mouseOverTop = false;
      }
      if (event === "mouseOverBottom") {
        state.mouseOverBottom = true;
      }
      if (event === "mouseOutBottom") {
        state.mouseOverBottom = false;
      }
      if (event === "topSelectorFocused") {
        state.topSelectorFocused = true;
      }
      if (event === "topSelectorBlured") {
        state.topSelectorFocused = false;
      }
      if (event === "bottomSelectorFocused") {
        state.bottomSelectorFocused = true;
      }
      if (event === "bottomSelectorBlured") {
        state.bottomSelectorFocused = false;
      }

      if (Object.values(state).some((x) => x)) {
        //if any of the states is true show selectors
        if (currentBlurTimeout) {
          //clearing pending blur if any
          clearTimeout(currentBlurTimeout);
          currentBlurTimeout = null;
        }
        this.showVerticalSelectors.emit("show");
      } else {
        // one of the blur events happend
        currentBlurTimeout = setTimeout(() => {
          // blur only after some time and doublecheck before blurring
          if (!Object.values(state).some((x) => x)) {
  ...
  private _createCoreObservable(): Observable<string> {
    let mousemoving = fromEvent(this.importedElements.chart, "mousemove")
      ...
      .pipe(
        map((val) => {
          // simplify to movement / no movement emits
          if (val.length === 0) {
            return 0;
          } else {
            return 1;
      ...
      .pipe(
        map((val) => {
          if (val === 0) {
            return "mouseStoppedMoving";
          } else {
            return "mouseStartedMoving";
          }
        })
      );

    let mouseOverChart = fromEvent(
      this.importedElements.chart,
      "mouseover"
    ).pipe(map((event) => "mouseOverChart"));
    let mouseOutChart = fromEvent(this.importedElements.chart, "mouseout").pipe(
      map((event) => "mouseOutChart")
    );

    let mouseOverTopSide = fromEvent(
      this.importedElements.selectors[0],
      "mouseover"
    ).pipe(map((event) => "mouseOverTop"));
    let mouseOutTopSide = fromEvent(
      this.importedElements.selectors[0],
      "mouseout"
    ).pipe(map((event) => "mouseOutTop"));

    let mouseOverBottomSide = fromEvent(
      this.importedElements.selectors[1],
      "mouseover"
    ).pipe(map((event) => "mouseOverBottom"));
    let mouseOutBottomSide = fromEvent(
      this.importedElements.selectors[1],
      "mouseout"
    ).pipe(map((event) => "mouseOutBottom"));

    let topSelectorFocused = fromEvent(
      this.importedElements.inputs[0],
      "focus"
    ).pipe(map((event) => "topSelectorFocused"));
    let topSelectorBlured = fromEvent(
      this.importedElements.inputs[0],
      "blur"
    ).pipe(map((event) => "topSelectorBlured"));

    let bottomSelectorFocused = fromEvent(
      this.importedElements.inputs[1],
      "focus"
    ).pipe(map((event) => "bottomSelectorFocused"));
    let bottomSelectorBlured = fromEvent(
      this.importedElements.inputs[1],
      "blur"
    ).pipe(map((event) => "bottomSelectorBlured"));

    return merge(
      //observable that emits whenever any of its inner observables emits
      mousemoving,
      // mouseOverChart,
      // mouseOutChart,
      mouseOverTopSide,
