...
import { DateTime, EpochNumber, TelemetryCachePoint, TelemetryPointReceived, TelemetrySource, TimediffControlUpdate } from '../types/custom.types';

@Injectable({ providedIn: 'root' })
export class UtilitiesService {

  timestampEpochnumberToDatestring(timestamp: number, keepDecimalMilliseconds = 1): string {
    let parts = this._timestampEpochnumberToDateParts(timestamp)
    parts.timePart = this.trimTimestringsMilliseconds(parts.timePart, keepDecimalMilliseconds)
    return `${parts.datePart} ${parts.timePart}`
  }

  //following function is used when updating rangeStart and rangeEnd controls:
  timestampEpochnumberToDateTime(timestamp: number, format: '12' | '24' = '24', keepDecimalMilliseconds = 3)
  ...
  private _transformToTwelveFormat(timepart: string) {
    ...
    if (!hours || !minutes || !seconds) {
      console.error('Error transforming to twelve timeformat, input:', timepart)
    }
    if (+hours === 0) {
      hours = '12'
      ending = ' PM'
  ...
  private _timestampEpochnumberToDateParts(timestamp: number) {
    ...
    return { datePart: isoParts[0], timePart: isoParts[1].substr(0, isoParts[1].length - 1) }
  }

  timestampDateTimeToEpochnumber(timestamp: DateTime): EpochNumber {
    return this.timestampDatestringToEpochnumber(`${timestamp.date} ${timestamp.time}`)
  }
  ...
  timestampEpochnumberToTimestring(
    ...
    let returnFormat = `${hours}:${minutes.padStart(2, '0')}:${seconds.padStart(2, '0')}`
    ...
    else { //keep only required number of decimal places
      let fullFormat = returnFormat + `.${milliseconds.padStart(3, '0')}`
      return this.trimTimestringsMilliseconds(fullFormat, keepDecimalMilliseconds)
    }
  }
  ...
  trimTimestringsMilliseconds(timestring: string, numberOfDecimalPointsToKeep) {
    ...
    if (indexOfDot !== -1) { //if timestring contains dot
      if (numberOfDecimalPointsToKeep > 0) {
        return timestring.slice(0, indexOfDot + numberOfDecimalPointsToKeep + 1)
      } else {
  ...
  trimZeroesFromTimestring(timestring: string) {
    ...
    for (let chr of timestring) {
      ...
      } else { // if you reached a character number > 0, we are done
        break;
      }
      i++;
    }
    if (finalString === '') { finalString = '0.0' }
  ...
  getNamesValuesOfModifiedControls(formObject: NgForm): { [controlName: string]: DateTime | TimediffControlUpdate } {
    let controlName, controlProperties;
    let namesValuesOfDirtyControls = {};
    for ([controlName, controlProperties] of Object.entries(formObject.form.controls)) {
  ...
  getNamesValuesOfControls(formObject: NgForm): { [name: string]: DateTime | { timediff: string } | string } {
    let controlName, controlProperties;
  ...
  checkIsPositiveInteger(inputString: string) {
  ...
  checkIsDatestring(inputString: string) {
    if (Date.parse(inputString)) {
      return true;
    } else {
      return false;
    }
  ...
  checkIsTimestring(inputString: string) {
    ...
    milliseconds = (milliseconds !== undefined) ? milliseconds.slice(0, 3) : undefined
    if (+milliseconds < 0 && milliseconds !== undefined) return false;
    return true;
  }

  subtractTimestringFromDatestring(datestring: string, timestring: string): string {
    if (!this.checkIsDatestring(datestring)) throw new Error('datestring expected, got this: ' + datestring);
    if (!this.checkIsTimestring(timestring)) throw new Error('timestring expected, got this: ' + timestring);
    let dateEpochnumber = this.timestampDatestringToEpochnumber(datestring)
    let timeEpochnumber = this.timestampTimestringToEpochnumber(timestring)
    let result = this.timestampEpochnumberToDatestring(dateEpochnumber - timeEpochnumber)
    return result;
  }
  subtractTimestringFromEpochnumber(substractFrom: EpochNumber, substractBy: string): EpochNumber {
    if (!this.checkIsPositiveInteger(substractFrom.toString())) throw new Error('epochnumber expected, got this: ' + substractFrom);
    if (!this.checkIsTimestring(substractBy)) throw new Error('timestring expected, got this: ' + substractBy);
    let substractByAsEpochnumber = this.timestampTimestringToEpochnumber(substractBy);
    let result = substractFrom - substractByAsEpochnumber;
    return result;
  }
  getPositionsOfDataEncodedInBuffer(buf: ArrayBuffer) {
    ...
    for (let i = dataView.byteLength - 1; i >= dataView.byteLength - 100; i--) {
      ...
      if (symbol === 44) { //if you find comma (ascii value for comma is 44)
        positionInBuf = buf.byteLength - 100 + i
        timestampAndNameCommaPosition.unshift(positionInBuf)
      }
      if (timestampAndNameCommaPosition.length === 2) { //when both commas are found
        break;
      }
    }
  ...
  transformToCacheWithEpochnumbers(cache: TelemetryCachePoint[]): TelemetryPointReceived[] {
    if (cache.length === 0) { return [] }
    return cache.map(arr => {
      let timestamp = (arr[0] as Date).getTime()
      let theRest = arr.slice(1)
      return [timestamp, ...theRest] as (number | Blob)[]
    })
  }
  transformToCacheWithDateObjects(cache: TelemetryPointReceived[]): TelemetryCachePoint[] {
    if (cache.length === 0) { return [] }
    return cache.map(arr => {
      let timestamp = new Date(arr[0] as EpochNumber)
      let theRest = arr.slice(1)
      return [timestamp, ...theRest] as (number | Date)[]
    })
  }
  extractSelectedSources(formObject: NgForm): TelemetrySource[] {
    let values = Object.entries(formObject.form.value)
    if (!values) return null;
    if (typeof (values[0][1]) === 'boolean') { //checkboxes
      return values
  ...
  calculateStepAndPrecision(currentRange: number, rate: number): [number, number] {
    let step = currentRange / rate;
    let roundingCoeficient = step > 1 ? 0 : Math.ceil(Math.abs(Math.log10(step)))
