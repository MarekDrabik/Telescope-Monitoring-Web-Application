...
export class OnClickFocusDirective implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.zIndexSubscription = this.layoutService.zIndexSubject.subscribe(
      (zIndexToId) => {
        if (Object.keys(zIndexToId).length !== 0) {
          this.renderer2.setStyle(
            this.el.nativeElement,
            "z-index",
            zIndexToId[this.appletId.toString()]
          );
        }
      }
    );
    ...
    this.unlisten = this.renderer2.listen(
      ...
      () => {
        ...
        if (this.el.nativeElement.tagName === "DIV") {
          //true for applet references in the header
  ...
  ngOnDestroy(): void {
    if (this.zIndexSubscription && this.zIndexSubscription.unsubscribe)
