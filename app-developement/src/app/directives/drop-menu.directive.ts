...
export class DropMenuDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.renderer2.listen(this.el.nativeElement, 'click', (event) => {
      ...
      this.renderer2.setStyle(subMenuAsElement, 'bottom', subMenuPosition.bottom)
  ...
  private _calculateSubmenuPosition() {
    ...
    let right = 'unset';
    let left = 'unset';
    switch (this.dropSide) { //additional sides need to be implemented if needed
      case 'bottom':
