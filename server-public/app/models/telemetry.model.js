...
const _realtimeMessageComposers = {
    ...
    'allPoints': function (sourceName, currentMeasurements) {
        return JSON.stringify({
            timestamp: currentMeasurements.timestamp,
    ...
    'image': function (sourceName, currentMeasurements) {
        ...
        let imageBuffer;
        if (settings.imagesAreStoredAsIndexes) { //image is index (number)
...
module.exports = class TelemetryModel {
    ...
    static getMessageComposer(sourceName) {
        ...
        let sourceType = sourceName === 'allPoints' ? 'allPoints' : TelemetryModel.getType(sourceName)
        return _realtimeMessageComposers[sourceType] //returns function
    }
